...
<head>
    ...
    <title>后台管理 - 修改<?php echo $this->tableComment; ?></title>
...
<div id="main" class="main" >
    <div class="content">
        <div class="title">编辑<?php echo $this->tableComment; ?> [ <a href="__CONTROLLER__/index">返回列表</a> ]</div>
        <form method='post' id="form" action="__CONTROLLER__/upd" >
...
foreach($this->fullFields as $field => $fieldInfo ) :
	...
	continue ; 
        ...
        $formItem = '<textarea  id="'.$field.'_id" name="'. $field .'" style="width:750px;height:300px;"><?php echo $data[\''.$field.'\']; ?></textarea>' ;
    } else if( strpos($fieldInfo['formItem'], 'textarea' ) ===0 ) {
        $typeItems[$field] = 'textarea' ;
        $formItem = '<textarea class="bLeft"  id="'.$field.'_id"  name="'. $field .'" rows="6" cols="65"><?php echo $data[\''.$field.'\']; ?></textarea>' ;
        ...
        foreach($fieldInfo['value'] as $k => $v) {
            $formItem .= '<option '.$selected.' value="'.$k.'">'.$v.'</option>' ;
        }
        $formItem .= '</select>' ;
    } else if( strpos($fieldInfo['formItem'], 'image' )   === 0) {      // 单图片上传, 图片上传一般要有预览功能
        $typeItems[$field] = 'image' ;
	$delImg = '&nbsp;&nbsp;&nbsp;<button type="button" class="delImg"> 删除图片 </button>' ;
        $formItem = '<input type="file"  id="'.$field.'_id" class="medium bLeftRequire"  name="'. $field.'" />' . $delImg ;
...
HTML;
        ...
        $formItem = '<input type="input"  readonly="readonly" id="'.$field.'_id" onclick="WdatePicker()"  name="'. $field.'" value="<?php echo date("Y-m-d", $data[\''.$field.'\']); ?>" />' ;
    } else if(strpos($fieldInfo['formItem'], 'password' ) === 0 ) {
	$typeItems[$field] = 'password' ;
	$formItem = '<input type="password" class="medium bLeftRequire" id="'.$field.'_id"  name="'. $field.'" />' ;
    } else if(strpos($fieldInfo['formItem'], 'input' ) === 0 ) {
	$formItem = '<input type="text" class="medium bLeftRequire" id="'.$field.'_id"  value="<?php echo $data[\''.$field.'\']; ?>" name="'. $field.'" />' ;
    } else {
        $formItem = '<input type="text" class="medium bLeftRequire" id="'.$field.'_id"  value="<?php echo $data[\''.$field.'\']; ?>"  name="'. $field.'" />' ;
    }
...
<?php endforeach; ?>
            <tr>
                <td></td>
                <td class="center">
                    <input type="submit" value="更 新" class="small submit">
                    <input type="reset" class="submit  small" value="重 置" >
                    <input type="hidden" value="<?php echo "<?php echo \$data['".$this->pk ."']; ?>" ?>" name="<?php echo $this->pk; ?>" />
                </td>
            </tr>
        </table>
        </form>
...
foreach($typeItems as $field => $type) :
    ...
    $("form #<?php echo $field; ?>_id").next('button.delImg').click(function() {
        $nowTr.next('.preview').remove();
        $("form #<?php echo $field; ?>_id").val('');
    });
    
    var img1 = '<?php echo '<?php echo $data[\'' . $field . '\']; ?>' ; ?>';
    if(img1) {
	var previewImg = '__ROOT__/Uploads/' + img1 ;
	// var hasFile = <?php echo  file_exists( '__ROOT__/Uploads/' . '<?php echo $data[\'' . $field . '\']; ?>' ); ?> ;
	$trHtml = '<tr class="preview"><td align="right">预览效果：</td><td><img width="250" height="180"  src="'+ previewImg +'"  /><input type="hidden" name="<?php echo $field; ?>" value="'+ img1 +'" /></td></tr>' ;
	$nowTr.after($trHtml);
    }
    function loading(){
...
<?php elseif($type === 'mimage') : ?>
    // 多图的显示
    $('form ul#pic_list').append('<?php echo '<?php echo $images ; ?>';  ?>');
    $('form #album').val('<?php echo '<?php echo $values ; ?>';  ?>');
    
    
<?php elseif( in_array($type, ['select', 'checkbox', 'input', 'radio']) ) : ?>
    $('form #<?php echo $field;?>_id').val('<?php echo '<?php echo $data[\''.$field.'\'] ; ?>';  ?>');
